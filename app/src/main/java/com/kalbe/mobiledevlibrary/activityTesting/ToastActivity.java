        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
               if (swipeLists.get(position).get_txtTitle().equals("Toast custom spg mobile true")){
                   new ToastCustom().showToastSPGMobile(getApplicationContext(), swipeLists.get(position).get_txtTitle(), true);
               } else if (swipeLists.get(position).get_txtTitle().equals("Toast custom spg mobile false")){
                   new ToastCustom().showToastSPGMobile(getApplicationContext(), swipeLists.get(position).get_txtTitle(), false);
                   ...
                   new ToastCustom().showToastyCustom(getApplicationContext(), swipeLists.get(position).get_txtTitle(), icon, Color.BLUE, 5, false, true);
               }
               else {
                   new ToastCustom().showToasty(getApplicationContext(), swipeLists.get(position).get_txtTitle(), position+1);
               }
            }
        });
    }
